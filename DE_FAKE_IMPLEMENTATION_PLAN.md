...
| **Dropper TLS + HTTP** | **No statically linked TLS** in the dropper. **Windows:** **WinHTTP** for HTTPS. **macOS:** **NSURLSession**. **Linux:** **libcurl** linked against **system** OpenSSL/LibreSSL (document minimum distro; CI Ubuntu 22.04+). **System CA trust only**; **no** public-key pinning in v1. |
| **Dropper implementation** | **C17, Zig 0.14+, or Rust** with **`panic=abort`**, **LTO**, **strip**, **no heavy async runtimes** (blocking I/O + small thread pool max). |
| **Dropper second instance** | Live `install_parent/install.lock` → **native GUI alert** “Another installer is running” + **exit 6**. |
| **Windows in-place update** | Keep **`versions/<semver>/` + `current` junction** for unpacked trees. To refresh **`agent-runtime.exe`**: **rename** in-use binary to **`agent-runtime.exe.old`** (allowed while running), copy **new** `agent-runtime.exe` beside it, **MessageBox** “Update installed—please restart Agent.” **Exit 7** + GUI only if rename/copy fails (ACL). |
| **Install visibility** | After successful install: create **`Documents/<ProductDisplayName>`** → **symlink/junction** to `install_parent/current`, **open folder** in Explorer/Finder/`xdg-open`, and **GUI dialog** explaining this folder is the portable copy unit. |
...
| **Atomic swap** | Download to `*.partial` then unpack into `install_parent/versions/<semver>/`; update `current` junction/symlink. **Windows exe update:** use **rename-in-use** pattern in **Frozen product choices** (not “user must stop first” as primary path). |
...
- **Exit codes are insufficient:** every user-visible failure path must show a **graphical alert** when GUI is available (see Frozen table). Log the same text to a file under **`Documents/<ProductDisplayName>/LAST_ERROR.txt`** on all OSes for support screenshots.
- **Success path:** after install, **always** run the **Documents junction + open folder + explanatory dialog** sequence (Frozen table). First-run README in that folder: **`README.txt`** one page: “This folder is your Agent—copy it to a USB drive to move computers.”
- **Double-click entry:** macOS = **`Agent.app`** only for consumer docs; Windows = **`Agent.exe`** or Start Menu shortcut pointing at `agent-runtime.exe` inside `install_root`—ship **one** branded shortcut in installer.
- **`--headless`:** dropper and runtime accept this flag for automation; **suppresses all GUI** (tests/IT only).

---

### 2.10 First-run configuration and prompts (stage 2)

Non-technical users do not configure JSON or env vars. **`agent-runtime run`** must implement:

1. **First-launch wizard (local HTML):** after vLLM is up, open the **default browser** to **`http://127.0.0.1:8765/setup`** (same host as router; **new route**, not authenticated in v1 beyond localhost). Serve a **single-page** form: language, optional API keys (stored in **OS keychain** / **Credential Manager** / **libsecret**—never plain `config.json` for secrets), and **high-level toggles** (e.g. “email monitoring on/off”) that map to **`user_config.json`** keys the router reads.
2. **`user_config.json`** (created by wizard, path **`install_root/user_config.json`**): JSON schema versioned; `agent-runtime` merges with `manifest.json` defaults. If missing on first `run`, redirect **all** HTTP GET `/` and `/v1/*` except `/setup` and `/health` to **`/setup`** until `user_config.json` exists **or** user clicks “Skip” (writes minimal defaults).
3. **Ongoing prompts:** the **`POST /v1/invoke`** contract remains for integrations; for humans, the **setup page** and a minimal **`http://127.0.0.1:8765/chat`** (optional v1) send the same payload shape the router already accepts—**no second protocol**.
4. **Tray / menu bar (optional v1.1):** if time permits, a tiny tray icon opens the same `/setup` URL; if not shipped, the success dialog must say “Open this link in your browser: …”.
...
| Fatal dropper errors show GUI (unless `--headless`) | User thinks app is “broken” with silent exit |
| Documents junction + folder open after install | User cannot find `%LOCALAPPDATA%` copy unit |
| macOS `Agent.app` + Windows `.ico` | Suspicion / Gatekeeper friction |
...
**Dropper staging** (same parent): `install_parent/staging/<uuid>/` for download + unpack; on success: move tree to `install_parent/versions/<version>/`, atomically update `current` (see §2.3.1b).

**Post-install UX (all OS):** create **`%USERPROFILE%\Documents\<ProductDisplayName>`** (Windows) / **`$HOME/Documents/<ProductDisplayName>`** (macOS/Linux) as a **junction/symlink** to `install_parent/current`, **open** that folder in the shell file manager, show **native success dialog** + **`README.txt`** stub. `<ProductDisplayName>` = marketing string (e.g. `Agent`).

### A.2 Stage-2 directory tree (normative)

**Install root** = directory that contains **`manifest.json`** (validator + runtime use this definition only).

#### macOS (consumer): inside `Agent.app`

```
Agent.app/
  Contents/
    Info.plist              # CFBundleName, CFBundleIconFile, LSMinimumSystemVersion
    MacOS/
      agent-runtime         # real binary; cwd set to Resources/runtime on launch
    Resources/
      runtime/              # ← install_root
        ...
        tools/
        models/
        user_config.json    # created by /setup wizard; gitignored in source builds
        logs/
```

#### Windows / Linux (no `.app`)

```
<install_root>/
  manifest.json
  FILES.sha256
  agent-runtime.exe         # Windows; icon embedded
  agent-runtime             # Linux
  python/
  roles/
  prefixes/
...
**`NEXUS_AGENT_INSTALL_ROOT`:** absolute path to **`…/Resources/runtime`** on macOS, or `<install_root>` on Windows/Linux. Dropper handoff: **macOS** `open -a Agent.app`; **Windows** `Agent.exe` shim or `agent-runtime.exe` with Start Menu shortcut; **Linux** `execv` `./agent-runtime`.
...
4. else **macOS:** if **`./Agent.app`** exists in cwd, set install root to **`./Agent.app/Contents/Resources/runtime`**
5. else resolve **`install_parent/current/manifest.json`** per §A.1
6. else **exit 1** (“no manifest”)

Commands:
  run [--foreground] [--no-browser]   Start vLLM + router; unless `--no-browser`, open **`http://127.0.0.1:8765/setup`** on first run if `user_config.json` missing
