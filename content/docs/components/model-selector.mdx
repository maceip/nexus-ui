...
`ModelSelectorContent` caps height with `min(var(--model-selector-content-max-height, 311px), var(--radix-dropdown-menu-content-available-height))`. Override the design max (before the viewport limit) by setting the custom property on `className`, e.g. `[--model-selector-content-max-height:500px]`.
