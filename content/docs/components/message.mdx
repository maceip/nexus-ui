...
#### Fenced code: Nexus **`CodeBlock`** replaces Streamdown’s default

**`MessageMarkdown`** passes **`components.code={CodeBlock}`** (`@/components/nexus-ui/codeblock`). Highlighting still comes from **`@streamdown/code`** and your **`Streamdown`** plugins—only the **UI** is custom.

**Compared to Streamdown’s built-in fenced block**, **`CodeBlock`** gives you:

- Figure, scroll viewport, and **`CodeBlock`** **`showTitleRow`** (optional; default **true** via **`CodeBlockChromeProvider`** or built-in defaults). Pass **`showTitleRow={false}`** on **`CodeBlock`** where you wire Streamdown’s **`components.code`** (or wrap: **`(props) => <CodeBlock {...props} showTitleRow={false} />`**) to hide the language header and float copy top-right. **`CodeBlockChromeProvider`** still sets defaults for **`keepBackground`**, **`allowCopy`**, and **`showTitleRow`** when those props are omitted on **`CodeBlock`**.
- **Line gutters** when **`lineNumbers`** is on, using global **`code .line`** / **`code .line::before`** (same idea as this site’s docs code block)

**Inline code** (single backticks in markdown) is rendered with **`components.inlineCode`** as a plain **`<code>`**, per [Streamdown’s `inlineCode` docs](https://streamdown.ai/docs/components#inline-code), so fenced blocks and inline spans stay separate.

**Install:** **`@nexus-ui/message`** adds **two** files in one go—**`message.tsx`** and **`codeblock.tsx`** (copy helpers live inside **`codeblock.tsx`**). Commit both. There is no **`@nexus-ui/codeblock`** package to add on its own.
